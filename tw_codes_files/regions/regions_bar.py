...
def sorting(regions_accounts):
    ...
    for i in sorted_values:
        for k in regions_accounts.keys():
            if regions_accounts[k] == i and not(k in sorted_dict):
...
regions_accounts = {'DE': 0, 'RU': 0, 'UA': 0, 'Japan': 0, 'China': 0, 'FR': 0, 'UK': 0}
...
index = ['Япония (Japan)', 'Китай (China)', 'Соед. Кор. (UK)', 'Россия (RU)', 'Украина (UA)', 'Франция (FR)', 'Германия (DE)'] # sorted_regions_accounts.keys()

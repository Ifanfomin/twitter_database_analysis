...
Идея заключается в том, чтобы одной строкой в консоли можно было получить нужный нам график. Для её реализации с помощью библиотеки argparse принимает введенный с консоли текст и прогоняем через условия, 
